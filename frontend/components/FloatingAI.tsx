...
function MessengerProductCard({ product, index, onJP, onZoom }: {
  product: ProductCard; index: number;
  onJP: (p: ProductCard) => void; onZoom: (p: ProductCard) => void;
}) {
  ...
  return (
    <div className={styles.messengerCard}>
      <div className={styles.messengerImageWrap} onClick={() => onZoom(product)}>
        <img src={product.image} alt={product.name} className={styles.messengerImage} />
      ...
      <div className={styles.messengerFooter}>
        <span className={styles.messengerPrice}>{product.price_ar.toLocaleString("fr-FR")} Ar</span>
        <button className={styles.jpBtn} onClick={() => onJP(product)}>🛒 Je Prends</button>
      </div>
    </div>
  );
}

function ConfirmPopup({ product, onOk, onCancel }: {
  product: ProductCard; onOk: () => void; onCancel: () => void;
}) {
  return (
...
export default function FloatingAI() {
  ...
  const handleSend = async () => {
    ...
    if (step === "ask_name") {
      const name = inputValue.trim(); setOrderName(name); addUserMessage(name); setInputValue("");
      setStep("ask_whatsapp"); setTimeout(() => addBotMessage("Votre numéro WhatsApp ? 📱"), 300); return;
    }
    if (step === "ask_whatsapp") {
      const wa = inputValue.trim(); setOrderWhatsapp(wa); addUserMessage(wa); setInputValue("");
      setStep("ask_location"); setTimeout(() => addBotMessage("Votre lieu de livraison ? (ville / quartier) 📍"), 300); return;
    }
    if (step === "ask_location") {
      const loc = inputValue.trim(); setOrderLocation(loc); addUserMessage(loc); setInputValue("");
      setStep("ask_payment");
      setTimeout(() => addBotMessage("Mode de paiement :\n1️⃣ MVola — 034 30 513 60 (Noeline)\n2️⃣ Orange Money\n3️⃣ Paiement à la livraison (WhatsApp)\n\nRépondez 1, 2 ou 3 😊"), 300); return;
    }
    if (step === "ask_payment") {
      const choice = inputValue.trim(); addUserMessage(choice); setInputValue("");
      const payLabels: Record<string, string> = { "1": "MVola", "2": "Orange Money", "3": "Paiement à la livraison" };
      const payLabel = payLabels[choice] || choice;
      const total = cart.reduce((s, i) => s + i.product.price_ar * i.quantity, 0);
      const itemsList = cart.map((i) => `• ${i.product.name} × ${i.quantity} — ${i.product.price_ar.toLocaleString("fr-FR")} Ar`).join("\n");
      setStep("done");
      setTimeout(() => addBotMessage(`🎉 Super ${orderName} ! Voici votre récapitulatif :\n\n${itemsList}\n\n💰 Total : ${total.toLocaleString("fr-FR")} Ar\n📍 Livraison : ${orderLocation}\n📱 WhatsApp : ${orderWhatsapp}\n💳 Paiement : ${payLabel}\n\nNous vous contacterons très bientôt sur WhatsApp pour confirmer ! Merci de votre confiance ✨`), 400); return;
    }

    const userText = inputValue.trim(); setInputValue(""); addUserMessage(userText); setIsLoading(true);
    try {
      ...
      if (result.products && result.products.length > 0) {
        ...
        result.products.forEach((p: ProductCard, i: number) => {
          setTimeout(() => setMessages((prev) => [...prev, { sender: "bot", singleProduct: p }]), (i + 1) * 400);
        });
      } else {
        setMessages((prev) => [...prev, { sender: "bot", text: result.response }]);
      }
    } catch {
      addBotMessage("Désolée, petit problème technique 😅 Réessayez ou contactez-nous sur WhatsApp !");
    } finally {
      setIsLoading(false);
    }
  };

  const handleJP = (product: ProductCard) => { setPendingProduct(product); setStep("confirm_item"); };

  const handleConfirmOk = () => {
    if (!pendingProduct) return;
    addItem({ id: pendingProduct.id, name: pendingProduct.name, price: pendingProduct.price_ar, image: pendingProduct.image, quantity: 1, category: pendingProduct.category });
    setCart((prev) => {
      const existing = prev.find((i) => i.product.id === pendingProduct.id);
      if (existing) return prev.map((i) => i.product.id === pendingProduct.id ? { ...i, quantity: i.quantity + 1 } : i);
      return [...prev, { product: pendingProduct, quantity: 1 }];
    });
    setPendingProduct(null); setStep("ask_more");
    const total = [...cart, { product: pendingProduct, quantity: 1 }].reduce((s, i) => s + i.product.price_ar * i.quantity, 0);
    setTimeout(() => addBotMessage(`✅ "${pendingProduct.name}" ajouté au panier !\n\nTotal actuel : ${total.toLocaleString("fr-FR")} Ar 🛍\n\nVous souhaitez :\n👉 Continuer vos achats — dites-moi ce que vous cherchez\n👉 Finaliser ma commande — tapez "commander"`), 300);
  };

  const handleConfirmCancel = () => {
    setPendingProduct(null); setStep("chat");
    setTimeout(() => addBotMessage("Pas de souci 😊 Vous voulez voir autre chose ?"), 200);
  };

  const handleSendWrapped = async () => {
    if (step === "ask_more" && inputValue.trim().toLowerCase().includes("commander")) {
      addUserMessage(inputValue.trim()); setInputValue("");
      if (cart.length === 0) { setTimeout(() => addBotMessage("Votre panier est vide 😊 Dites-moi ce qui vous intéresse !"), 200); setStep("chat"); return; }
      setStep("ask_name"); setTimeout(() => addBotMessage("Super ! Pour finaliser, votre nom complet SVP 😊"), 300); return;
    }
    if (step === "ask_more") setStep("chat");
    await handleSend();
  };

  // ── JSX du chat (utilisé dans le portal) ─────────────────────────────────
  ...
  const chatContent = (
    <div className={styles.chatCard}>
      ...
      {/* Confirmation popup — à l'intérieur du chatCard */}
      {step === "confirm_item" && pendingProduct && (
        <ConfirmPopup product={pendingProduct} onOk={handleConfirmOk} onCancel={handleConfirmCancel} />
      )}
        ...
        <div className={styles.messagesContainer}>
          {messages.map((msg, index) => (
            <div key={index}>
              ...
              {msg.singleProduct && (
                <MessengerProductCard product={msg.singleProduct} index={index} onJP={handleJP} onZoom={setZoomedProduct} />
              )}
              {msg.products && msg.products.length > 0 && (
                <div className={styles.productCards}>
                  {msg.products.map((p) => (
                    <MessengerProductCard key={p.id} product={p} index={0} onJP={handleJP} onZoom={setZoomedProduct} />
                  ))}
                </div>
              )}
            </div>
      ...
      <div className={styles.footer}>
        ...
        />
        <button onClick={handleSendWrapped} className={styles.sendBtn} disabled={isLoading || step === "confirm_item"}>
  ...
  return (
    <>
      {/* Wrapper pour trigger + hint — toujours dans le flow normal */}
      ...
      {/* Chat — portal dans document.body, totalement hors du wrapper */}
      {mounted && isOpen && createPortal(chatContent, document.body)}
